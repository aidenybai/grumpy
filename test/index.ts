import Grumpy from './../dist/index.js';
import assert from 'assert';

const test = (desc: string, fn: any) => {
	try {
		fn();
	} catch (err) {
...
test('filter items from the Group', () => {
	...
	group.set('b', 2);
	group.set('c', 3);
	const filtered = group.filter((x: any) => x % 2 === 1);
...
test('Cut a Grumpy into two Groups', () => {
	...
	group.set('e', 5);
	group.set('f', 6);
	const [ even, odd ] = group.cut((x: any) => x % 2 === 0);
...
test('Map items in a Group into an array', () => {
	...
	group.set('b', 2);
	group.set('c', 3);
	const mapped = group.map((x: any) => x + 1);
...
test('Check if some items pass a predicate', () => {
	...
	group.set('b', 2);
	group.set('c', 3);
	assert.ok(group.some((x: any) => x === 2));
});
...
test('Check if every items pass a predicate', () => {
	...
	group.set('b', 2);
	group.set('c', 3);
	assert.ok(!group.every((x: any) => x === 2));
});
...
test('Reduce Group into a single value with initial value', () => {
	...
	group.set('b', 2);
	group.set('c', 3);
	const sum = group.reduce((a: any, x: any) => a + x, 0);
...
test('Reduce Group into a single value without initial value', () => {
	...
	group.set('b', 2);
	group.set('c', 3);
	const sum = group.reduce((a: any, x: any) => a + x);
	assert.strictEqual(sum, 6);
});
...
test('Iterate over each item', () => {
	...
	group.set('c', 3);
	const a: any[] = [];
	group.each((v: any, k: any) => a.push([ k, v ]));
...
test('Sort a Group', () => {
	...
	const sorted = group.sort((a: any, b: any) => a - b);
